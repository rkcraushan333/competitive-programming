...
int f(int w, unordered_map<int, vp64> &mp, v64 &v1, vector<v64> &dp, int i = 0)
{
    if (i == v1.size() || w <= 0)
        return 0;
    int &t = dp[i][w];
    if (t != -1)
        return t;
    int ans = 0;
    ans = max(ans, f(w, mp, v1, dp, i + 1));
    for (auto x : mp[v1[i]])
    {
        if (x.first <= w)
        {
            ans = max(ans, x.second + f(w - x.first, mp, v1, dp, i + 1));
        }
    }
    return t = ans;
}
void inforkc()
{
    ...
    int w;
    cin >> w;
    vector<v64> dp(v1.size(), v64(w + 1, -1));
    cout << f(w, mp, v1, dp);
}
