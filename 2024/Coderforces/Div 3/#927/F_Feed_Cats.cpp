...
int f(v64 &v1, v64 &v2, v64 &v3, v64 &dp, int i = 0)
{
    if (i >= v1.size())
    {
        return 0;
    ...
    int ans = 0;
    // not take
    ans = max(ans, f(v1, v2, v3, dp, i + 1));
    // take
    int curr = v2[i];
    int x = upper_bound(v1.begin(), v1.end(), curr) - v1.begin();
    x--;
    int y = upper_bound(v1.begin(), v1.end(), v3[x]) - v1.begin();
    ans = max(ans, x - i + 1 + f(v1, v2, v3, dp, y));
    return rkc = ans;
}
void inforkc()
{
