...
// by inforkc
int f(v64 &v, int i, int j, vector<v64> &dp, bool b = 1)
{
    if (i > j)
        return 0;
    int &rkc = dp[i][j];
    ...
    {
        ans = -1e12;
        ans = max(ans, v[i] + f(v, i + 1, j, dp, b ^ 1));
        ans = max(ans, v[j] + f(v, i, j - 1, dp, b ^ 1));
    }
    else
    {
        ans = 1e12;
        ans = min(ans, f(v, i + 1, j, dp, b ^ 1));
        ans = min(ans, f(v, i, j - 1, dp, b ^ 1));
    }
    return rkc = ans;
}
void inforkc()
