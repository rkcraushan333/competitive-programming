...
// by inforkc
int f(int n, int a = 0, int b = 0)
{
    int ans = 0;
    if (a == 0 && b == 0)
    {
        ans += f(n - 1, 1, 1); // open 1*1
        ans += f(n - 1, 0, 0); // closed 1*1
        ans += f(n - 1, 0, 1); // 1*1 closed and open
        ans += f(n - 1, 1, 0); // 1*1 open and closed
        ans += f(n - 1, 2, 2); // 2*2 open
        ans += f(n - 1, 0, 0); // 2*2 closed
    }
    else if (a == 1 && b == 1)
    {
        ans += f(n - 1, 1, 1); // open 1*1
