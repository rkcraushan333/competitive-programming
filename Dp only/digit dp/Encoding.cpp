...
{
    if (!tight)
        return power10(s.size() - idx);
    if (idx == s.size())
        return 1;
    int &t = cntdp[idx][tight];
    if (t != -1)
        return t;
...
int dp[100001][2][11];
int f(string &s, int idx = 0, int tight = 1, int pre = 10)
{
    if (idx == s.size())
        return 0;
    int &t = dp[idx][tight][pre];
    if (t != -1)
        return t;
    int limit = tight ? s[idx] - '0' : 9;
    int ans = 0;
    forn(i, 0, limit + 1)
    {
        if (pre != i)
        {
            int contr = ((i * power10(s.size() - 1 - idx)) % mod) * cnt(s, idx + 1, tight & (limit == i));
            contr %= mod;
            ans += contr;
            ans %= mod;
        }
        ans += f(s, idx + 1, tight & (limit == i), i);
        ans %= mod;
    }
    return t = ans;
}
...
void inforkc()
{
    ...
    string l, r;
    cin >> nl >> l >> nr >> r;
    cout << ((((f1(r) + mod) % mod - (f1(l) + mod) % mod) + mod) % mod + bruteForce(l)) % mod << ln;
}
signed main()
{
