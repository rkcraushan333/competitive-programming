...
// by inforkc
bool f(int m, int i, map<char, v64> &mp, string &l, string &r, vector<v64> &dp, int j = 0)
{
    if (i == m)
    {
        return 0;
    ...
    {
        int t = lower_bound(mp[c].begin(), mp[c].end(), j) - mp[c].begin();
        if (t == mp[c].size())
        {
            ans = 1;
            break;
        ...
        {
            ans |= f(m, i + 1, mp, l, r, dp, mp[c][t] + 1);
        }
    }
    return rkc = ans;
}
...
void inforkc()
{
    ...
    vector<v64> dp(s.size() + 1, v64(m + 1, -1));
    if (f(m, 0, mp, l, r, dp))
        yy;
    else
        nn;
