...
int dx[4] = {0, 0, 1, -1};
int dy[4] = {1, -1, 0, 0};
void dfs(vector<vector<int>> &v, vector<vector<bool>> &vis, int i, int j, int &curr)
{
    if (i < 0 || j < 0 || i > v.size() - 1 || j > v[0].size() - 1 || vis[i][j] || v[i][j] == 0)
        return;
    vis[i][j] = 1;
    curr += v[i][j];
    for (int k = 0; k < 4; k++)
    {
        dfs(v, vis, i + dx[k], j + dy[k], curr);
    }
}
void inforkc()
{
