...
// by inforkc
void dfs(int src, int par, vector<v64> &adj, map<pair<int, int>, int> &m, int &ans, int level)
{
    ans = max(ans, level);
    for (auto child : adj[src])
    {
        ...
        if (par == -1)
        {
            dfs(child, src, adj, m, ans, level);
        }
        else if (m[{src, child}] > m[{src, par}])
        {
            dfs(child, src, adj, m, ans, level);
        }
        else
        {
            dfs(child, src, adj, m, ans, level + 1);
        }
    }
}
void inforkc()
