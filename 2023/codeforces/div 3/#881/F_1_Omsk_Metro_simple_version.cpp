...
void inforkc()
{
    ...
    forn(i, 0, nt)
    {
        ...
        if (cd == '+')
        {
            ...
            if (b == 1)
            {
                v[curr] = {min(pn[a].first, v[a].first), max(v[a].second, pn[a].second + 1)};
                pn[curr] = {0, pn[a].second + 1};
            }
            else
            {
                v[curr] = {min(pn[a].first - 1, v[a].first), max(pn[a].second, v[a].second)};
                pn[curr] = {pn[a].first - 1, 0};
            }
        }
        else
        {
