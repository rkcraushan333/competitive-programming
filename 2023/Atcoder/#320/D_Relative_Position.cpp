...
void inforkc()
{
    ...
    while (st.size())
    {
        v64 v = *st.begin();
        st.erase(v);
        if (vis.count(v[0]) && vis.count(v[1]))
        {
            // do nothing
        }
        else if (vis.count(v[0]))
        {
            pair<int, int> vv = ans[v[0]];
            ans[v[1]] = {v[2] + vv.first, v[3] + vv.second};
            vis.insert(v[1]);
        }
        else if (vis.count(v[1]))
        {
            pair<int, int> vv = ans[v[1]];
            ans[v[0]] = {vv.first - v[2], vv.second - v[3]};
            vis.insert(v[0]);
        }
        if (st.empty())
        {
            ...
            for (auto v1 : temp)
            {
                if (vis.count(v1[0]) && vis.count(v1[1]))
                    continue;
                else if (vis.count(v1[0] || vis.count(v1[1])))
                {
                    st.insert(v1);
                    st1.erase(v1);
                }
