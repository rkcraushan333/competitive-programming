...
int inv(int n)
{
    return fastexpo(n, mod - 2, mod) % mod;
}
// for inverse modulo (k^mod-2)%mod
// by inforkc
pair<int, int> f(v64 &v, vector<vector<vp64>> &dp, string &s, int i, int j, int k, int id = 0)
{
    if (id == k || (i == j))
    {
        return {0, 1};
    }
    pair<int, int> rkc = dp[i][j][id];
    pair<int, int> cc = {-1, -1};
    if (rkc != cc)
        return rkc;
    ...
    if (s[id] == 'L')
    {
        ...
        for (int x = i; x < j; x++)
        {
            curr += v[x];
            cout << curr << "a" << endl;
            pair<int, int> t = f(v, dp, s, i, x, k, id + 1);
            ans.first += t.first + curr;
            ans.first %= mod;
            ans.second += t.second;
        }
    }
    else
    {
        ...
        for (int x = j; x > i; x--)
        {
            curr += v[x];
            cout << curr << "b" << endl;
            pair<int, int> t = f(v, dp, s, x, j, k, id + 1);
            ans.first += t.first + curr;
            ans.first %= mod;
            ans.second += t.second;
        }
...
void inforkc()
{
    ...
    string s;
    cin >> s;
    vector<vector<vp64>> dp(n + 1, vector<vp64>(n + 1, vp64(k + 1, {-1, -1})));
    pair<int, int> p = f(v, dp, s, 0, n - 1, k);
    cout << (p.first * (inv(p.second)) % mod) << ln;
}
