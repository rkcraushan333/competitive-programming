...
// by inforkc
int dfs1(unordered_map<int, int> &mp, vector<v64> adj, int s, int p)
{
    if (adj[s].size() == 0)
    {
        mp[s] = 0;
    ...
    for (auto child : adj[s])
    {
        if (child != p)
        {
            cnt += dfs1(mp, adj, child, s);
        }
    }
    mp[s] = cnt;
    return cnt;
}
int dfs2(unordered_map<int, int> &mp, vector<v64> adj, int s, int p, int &ans)
{
    if (adj[s].size() == 0)
    {
        return 1;
    ...
    for (auto child : adj[s])
    {
        if (child != p)
        {
            cnt = min(cnt, dfs2(mp, adj, child, s, ans));
        }
    }
    return cnt + 1;
}
