...
int hdfs(vector<v64> &adj, vp64 &height, int s, int par)
{
    int a = 0, b = 0;
    for (int child : adj[s])
    {
...
void dfs(vector<v64> &adj, vp64 &h, v64 &ans, int s, int par, int p_ans = 0)
{
    ans[s] = max(p_ans, h[s].first);
    for (int child : adj[s])
    {
        if (child != par)
        {
            if (h[child].first + 1 == h[s].first)
                dfs(adj, h, ans, child, s, max(p_ans, h[s].second) + 1);
            else
                dfs(adj, h, ans, child, s, ans[s] + 1);
        }
    }
}
void inforkc()
{
    ...
    // for (int i = 1; i <= n; i++)
    // {
    //     cout << i << "->" << height[i].first << "," << height[i].second << ln;
    // }
    v64 ans(n + 1);
    dfs(adj, height, ans, 1, -1);
    forn(i, 1, n + 1)
    {
        cout << ans[i] << " ";
    }
