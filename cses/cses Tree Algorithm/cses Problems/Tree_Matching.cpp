...
int dfs(vector<v64> &adj, int s, int par, bool taken = 0)
{
    if (adj[s].size() == 0 && (s != 1))
    {
        return 0;
    ...
    for (int child : adj[s])
    {
        if (child != par)
        {
            if (taken == 0)
            {
                int a = dfs(adj, child, s, 0);
                mp[child] = a;
                x += a;
            }
            else
            {
                ans += dfs(adj, child, s, 0);
            }
        }
    }
    for (int child : adj[s])
    {
        if (child != par)
        {
            if (taken == 0)
            {
                ans = max(ans, x - mp[child] + 1 + dfs(adj, child, s, 1));
            }
        }
    }
    return dp[{s, taken}] = ans;
}
void inforkc()
{
    int n;
