...
void inforkc()
{
    ...
    set<v64> pq;
    v64 dis(n + 1, INT64_MAX); // sum can be more than INT_MAX
    dis[1] = 0;
    pq.insert({0, 1});
    while (pq.size())
    {
        ...
        for (auto c : adj[t[1]])
        {
            if (dis[c.first] > t[0] + c.second)
            {
                if (pq.count({dis[c.first], c.first}))
                    pq.erase({dis[c.first], c.first});
                dis[c.first] = t[0] + c.second;
                pq.insert({dis[c.first], c.first});
            }
        }
    }
    for (int i = 1; i <= n; i++)
