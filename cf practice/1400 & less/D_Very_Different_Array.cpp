...
void inforkc()
{
    ...
    int i1 = 0, j1 = n - 1, i2 = 0, j2 = m - 1, ans = 0;
    while (i1 <= j1)
    {
        int a = max(abs(v1[i1] - v2[i2]), abs(v1[j1] - v2[i2]));
        int b = max(abs(v1[i1] - v2[j2]), abs(v1[j1] - v2[j2]));
        if (a >= b)
        {
            ans += a;
            if (abs(v1[i1] - v2[i2]) == a)
            {
                i1++;
            }
            else
